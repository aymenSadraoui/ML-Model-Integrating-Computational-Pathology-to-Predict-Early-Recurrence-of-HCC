...
list_train_patients = [patient for patient in os.listdir(features_paths) if int(patient.split('_')[0][:-1])]
...
df1 = pd.read_excel(labels_path, sheet_name="BJN").dropna(subset=["Patient"])
list_train_patients = os.listdir(features_paths)
for feature_file in os.listdir(features_paths):
    if feature_file.endswith('_features.pt'):
        slide_id = feature_file.split('_')[0]
...
for model in models:
    ...
    results_dict[model.__class__.__name__] = [mean_f1,std_f1,mean_acc, std_acc, HM_test_acc, HM_test_f1, BJN_test_acc, BJN_test_f1]


# save results to a csv file
results_df = pd.DataFrame.from_dict(results_dict, orient='index', columns=['Mean F1 Score', 'Std F1 Score', 'Mean Accuracy', 'Std Accuracy', 'HM Test Accuracy', 'HM Test F1 Score', 'BJN Test Accuracy', 'BJN Test F1 Score']) 
